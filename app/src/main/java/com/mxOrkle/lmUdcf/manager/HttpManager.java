...
import com.mxOrkle.lmUdcf.model.RequestModel;
import com.mxOrkle.lmUdcf.model.SDKConfigModel;
import com.mxOrkle.lmUdcf.lwNm.AdError;
...
public class HttpManager {
    ...
    public  void updateNi(int nextTime,String appid){
        ...
        if (hbTime == 0l || ((currenTime-hbTime)/1000) >= nextTime){
            checkNiBefore(appid);
        }
    }
...
//        messageObjects.arg1 = ts;
    ...
    public void checkNiBefore(final String appid){
     ...
     * 配置信息初始化
     */
    public void requestNi(String appid){
        ...
        if (CommonUtils.isNetworkAvailable(mContext)){
            ...
            Map<String,String> params = getHbParams(appid);
...
//                    requestVGD();
            ...
            LogUtils.i(MConstant.TAG,"ni network unavaiable......");
        }
    }


    public Map<String,String> getHbParams(String appid){
        if (mDeviceInfo == null){
            mDeviceInfo = CommonUtils.readParcel(mContext,MConstant.DEVICE_FILE_NAME);
        }
...
//        }
    ...
    private static String replaceReportEventUrl(AdModel adModel,String originUrl,Context context){
        ...
        if (originUrl.contains("%%CLICKID%%")){
            originUrl = originUrl.replace("%%CLICKID%%",adModel.getClickid()==null? "":adModel.getClickid());
        }
        return originUrl;
    }
}
